...
if connexion.is_connected() :
    ...
    for id in urls :
        ...
        if not os.path.isfile(rep+'/'+id+'.csv') or time.time() - os.path.getctime(rep+'/'+id+'.csv') > 2592000 : #2592000 = nombre de sec dans un mois
            ...
            metadonnees = requests.get('https://www.data.gouv.fr/api/2/datasets/'+urls[id][0]).json()
            ...
            if if_file and os.path.isfile(rep+'/'+id+'.csv'):
                ligne = recup_infos[recup_infos["NOM"] == id]      # Retient seulement la ligne du fichier csv
            ...
            if not if_file or recup_version != version :
                ...
                lien = 'https://www.data.gouv.fr/fr/datasets/r/'+urls[id][1]
                ...
                nom_du_fichier = os.path.join(rep, id+'.csv')
     ...
     pour l'instant sur le terminal
                ...
                if pourcentage < 10 :
                    print (pourcentage,"%   -  ", id, "-> Fichier téléchargé")
        ...
        # avec gestion du nombre de caractères pour avoir du texte homogène
            ...
            pourcentage = int(csv_courant/nbr_csv*100)
            if pourcentage < 10 :
                print (pourcentage,"%   -  ", id, "-> Fichier à jour")
            elif pourcentage < 100 :
                print (pourcentage,"%  -  ", id, "-> Fichier à jour")
            else :
                print (pourcentage,"% -  ", id, "-> Fichier à jour")
...
if modified :
    w = csv.writer(open(rep+'/'+'versions.csv', "a")) # le "a" c'est l'équivalent de .append() pour les tableaux
